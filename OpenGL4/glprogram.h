...
public:
	...
	bool bind_fragment_data_location(const CString& _var_name,GLuint _color_number,GLuint _index = -1)
	{
		string_converter<TCHAR,GLchar> var_name(_var_name,CP_ACP);
		if(_index==-1)
			glBindFragDataLocation(program,_color_number,(const GLchar*)var_name);
		else
			glBindFragDataLocationIndexed(program,_color_number,_index,(const GLchar*)var_name);
