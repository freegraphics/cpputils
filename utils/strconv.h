										   ...
										   )
{
	...
	VERIFY_EXIT(!::IsBadStringPtrA(_src,srclen)
		&& NOT_NULL(_defbuf) 
		&& NOT_NULL(_src)
		);
	int len = ::MultiByteToWideChar(_codepage,0,_src,srclen,NULL,0);
	_destlen = len;
	_dest = (WCHAR*)((len+1>_defbufsize)?trace_alloc((new WCHAR[len+1])):_defbuf);
	ZeroMemory(_dest,sizeof(WCHAR)*(len+1));
	::MultiByteToWideChar(_codepage,0,_src,srclen,_dest,len+1);
	_dest[_destlen] = 0;
	_bdirect = _dest==_defbuf;
}
										   ...
										   )
{
	...
	VERIFY_EXIT(!::IsBadStringPtrW(_src,srclen)
		&& NOT_NULL(_defbuf) 
		&& NOT_NULL(_src)
		);
	int len = ::WideCharToMultiByte(_codepage,0,_src,srclen,NULL,0,NULL,NULL);
	_destlen = len;
	_dest = (len+1>_defbufsize)?trace_alloc(new CHAR[len+1]):(CHAR*)_defbuf;
	ZeroMemory(_dest,sizeof(CHAR)*(len+1));
	::WideCharToMultiByte(_codepage,0,_src,srclen,_dest,len+1,NULL,NULL);
	_dest[_destlen] = 0;
	_bdirect = _dest==_defbuf;
}
...
struct string_converter
{
	...
	*/
	string_converter(const _FromType* _source,int _srclen ,UINT _codepage)
		:m_pdata(NULL)
		,m_bdirect(false)
		,m_len(0)
	{
...
//#if _MSC_VER <= 1100
	...
	string_converter(const CString_& _str,UINT _codepage)
		:m_pdata(NULL)
		,m_bdirect(false)
		,m_len(0)
	{
...
//#endif //#if _MSC_VER <= 1100
	...
	*/
	...
	_ToType*& operator & () {return m_pdata;}
...
// 	\param _codepage -- code page used in conversation 
// 	*/
// 	string_converter(const CString_& _source,UINT _codepage = CP_ACP)
// 		:m_pdata(NULL)
// 		,m_bdirect(false)
// 		,m_len(0)
// 	{
	...
	*/
	...
	public:
		...
		*/
		...
		{
			size_t sz = get_maxBufLenToFormat(string_converter<TCHAR,CHAR>(_lpszFormat,-1,CP_ACP),_argsList);
			LPTSTR buf = GetBuffer((int)sz);
#if _MSC_VER >= 1300
	#if defined(_UNICODE)
			vswprintf_s(buf,sz,_lpszFormat,_argsList);
	#else
			vsprintf_s(buf,sz,_lpszFormat,_argsList);
	#endif
#else
			_vsntprintf(buf,sz,_lpszFormat,_argsList);
#endif
...
{
	string_converter<TCHAR,WCHAR> strW(_str,CP_ACP);	//_bufsize = 1024*4 (8kb or WCHAR[4*1024])
	outputW(strW,strW.get_length());
	string_converter<TCHAR,CHAR,1024*6> strA(_str,CP_ACP);	//_bufsize = 1024*6 (6kb or CHAR[1024*6])
