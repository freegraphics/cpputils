...
namespace common
{
	...
	*/
	...
	{
		string_converter<WCHAR,TCHAR,1024> str(_sz,-1,CP_ACP);
		_s = str;
		return true;
	}
	...
	*/
	...
	{
		string_converter<CHAR,TCHAR,1024> str(_sz,-1,CP_ACP);
		_s = str;
		return true;
	}
...
#include <comutil.h>
	...
	*/
	...
	{
		string_converter<WCHAR,TCHAR,1024> str(_bstr,-1,CP_ACP);
		_s = str;
		return true;
	}
	...
	{
		_s = _time.Format(_T("%Y %m %d %H %M %S"));
		return true;
	}

	inline bool convert(const CString_& _s,CTime_& _time)
	{
		long y = _time.GetYear(),m = _time.GetMonth(),d = _time.GetDay()
			,H = _time.GetHour(),M = _time.GetMinute(),S = _time.GetSecond()
			;
		_stscanf_s(_s,_T("%d %d %d %d %d %d"),&y,&m,&d,&H,&M,&S);
		_time = CTime_(y,m,d,H,M,S);
		return true;
	}
