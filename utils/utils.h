...
#if !defined(VERIFY_RETURN)
/**\brief macro that define verify and if fail exit (with specified return value)
\param _arg -- argument or expression to verify
\param _ret -- return value or expression
*/
#define VERIFY_RETURN(_arg,_ret)	\
{	\
	bool _ = (_arg);	\
	ASSERT_(_);	\
	if(!_) return (_ret);	\
...
_Type max_value(const _Type& _v1,const _Type& _v2)
{
...
template<typename _Type> inline
_Type min_value(const _Type& _v1,const _Type& _v2)
{
	if(_v1<_v2) return _v1;
	else return _v2;
}
...
{
	return (_Type)(_d1*(_cf) + _d2*(1.0-_cf));
}

template<typename _Type> inline
_Type between(const _Type& _d1,const _Type& _d2,float _cf)
{
	return (_Type)(_d1*(_cf) + _d2*(1.0f-_cf));
}
...
public:
	/**\brief function to return single const value for module of the specified class
	*/
	...
	static _Type& get_non_const()
	{
		return get_instance();
	}

	/**\brief function that return single non const value for module of the specified class
	*/
	static _Type& set()
	{
		return get_instance();
...
{	\
	struct ERROR_##_msg {};	\
	ERROR_##_msg d1; \
	ERROR_FOR_CLASS<_struct> d2;	\
	ERROR_FOR_FIELD<_field> d3;	\
	(void)sizeof( CompileTimeChecker<(_expr)!=0>( d1 ) );	\
	(void)sizeof( CompileTimeChecker<(_expr)!=0>( d2 ) );	\
	(void)sizeof( CompileTimeChecker<(_expr)!=0>( d3 ) );	\
}
...
public:
	...
	bool token(CString& _str)
	{
		PositionPairType pos;
		if(!token(pos)) return false;
		_str = CString(m_sString+pos.first,pos.second-pos.first);
		return true;
	}

	bool next(PositionPairType& _pos)
	{
		return token(_pos);
	}

	bool next(CString& _str)
	{
		return token(_str);
	}

	LPCTSTR begin(const PositionPairType& _pos) const 
	{
		return m_sString + _pos.first;
	}

	LPCTSTR end(const PositionPairType& _pos) const
	{
		return m_sString + _pos.second;
	}
...
public:
	...
	FileTokenizer(const CString _file_name,LPCTSTR _sDeviders,size_t _buffer_size = 64*1024)
		...
		,readed(0)
	{
		...
		else
		{
			...
			::SetFilePointer(m_hFile,m_file_pos,NULL,FILE_BEGIN);
		}
		m_nDividersLength = IS_NULL(m_sDividers)?0:lstrlen(m_sDividers);
	}
	...
	void close()
	{
		if(NOT_NULL(m_hFile) && NEQL(m_hFile,INVALID_HANDLE_VALUE))
		{
			::CloseHandle(m_hFile);
			m_hFile = NULL;
		}
	...
	bool token(PositionPairType& _pos)
	{
		...
		// exit if all string passed
		if(m_file_size==0 || eof()) return false;

		// set last end position to current position
		m_nPos = m_nEndPos;

		reload_if_need(m_nPos,true);

		// skip all dividers
		for(
			;!eof()
				&& NEQL(m_buffer[m_nPos],0)
				&& find_char(m_sDividers,m_nDividersLength,m_buffer[m_nPos])
			;m_nPos++
			)
		{
			reload_if_need(m_nPos,true);
		...
		if(!eof())
		{
			VERIFY_EXIT1(NEQL(m_buffer[m_nPos],0),false);
		}

		// exit if all passed
		if(eof()) return false;

		reload_if_need(m_nEndPos);

		// skip all not dividers
		for(
			;!eof()
				&& NEQL(m_buffer[m_nEndPos],0)
				&& !find_char(m_sDividers,m_nDividersLength,m_buffer[m_nEndPos])
			;m_nEndPos++
			)
		{
			reload_if_need(m_nEndPos);
		...
		if(!eof())
		{
			VERIFY_EXIT1(NEQL(m_buffer[m_nEndPos],0),false);
		}
	...
	bool token(CString& _str)
	{
		PositionPairType pos;
		if(!token(pos)) return false;
		_str = CString((LPCTSTR)&(m_buffer[0])+pos.first,pos.second-pos.first);
		return true;
	}
	...
	LPCTSTR begin(const PositionPairType& _pos) const 
	{
		VERIFY_EXIT1(!m_buffer.empty(),NULL);
		return (LPCTSTR)&(m_buffer[0]) + _pos.first;
	}

	LPCTSTR end(const PositionPairType& _pos) const
	{
		VERIFY_EXIT1(!m_buffer.empty(),NULL);
		return (LPCTSTR)&(m_buffer[0]) + _pos.second;
	}
	...
	bool eof() const 
	{
		if(m_hFile==INVALID_HANDLE_VALUE || IS_NULL(m_hFile)) return true;
		DWORD pos = m_nPos;
		VERIFY_DO(pos<=readed,pos=readed);
		return m_file_pos + (long)pos>=m_file_size;
	}

protected:
	void reload_if_need(DWORD& _pos,bool _can_change_position = false)
	{
		VERIFY_EXIT(_pos<MAXINT-1);
		if(_pos+1<m_buffer.size() && !(m_buffer.size()>m_buffer_size && _can_change_position)) return; // do nothing if we don`t need to reload data
		reload(_pos,_can_change_position);
	}

	void reload(DWORD& _pos,bool _can_change_position = false)
	{
		if(eof()) return;
		DWORD size = m_buffer_size;
		if(_can_change_position)
		{
			m_file_pos += _pos;
			_pos = 0;
			size = m_buffer_size;
		...
		{
			size = m_buffer.size() + m_buffer_size;
		}
		::SetFilePointer(m_hFile,m_file_pos,NULL,FILE_BEGIN);
		VERIFY_EXIT(size!=0);
		m_buffer.resize(size);
		DWORD dwReaded = 0;
		readed = min((long)m_buffer.size(),m_file_size-m_file_pos);
		::ReadFile(m_hFile,&(m_buffer[0]),readed,&dwReaded,NULL);
	}
};//class FileTokenizer
...
inline
int wait4event(HANDLE _h1,DWORD _millisec)
{
	DWORD wait_res = ::WaitForSingleObject(_h1,_millisec);
...
inline
int wait4event(HANDLE _h1,HANDLE _h2,DWORD _millisec)
{
	HANDLE h[] = {_h1,_h2};
	DWORD wait_res = ::WaitForMultipleObjects(_countof(h),h,FALSE,_millisec);
	if(wait_res>=WAIT_OBJECT_0 && wait_res<WAIT_OBJECT_0+MAXIMUM_WAIT_OBJECTS)
		return wait_res - WAIT_OBJECT_0;
	else
		return -1;
}
...
inline
int wait4event(HANDLE _h1,HANDLE _h2,HANDLE _h3,DWORD _millisec)
{
	HANDLE h[] = {_h1,_h2,_h3};
	DWORD wait_res = ::WaitForMultipleObjects(_countof(h),h,FALSE,_millisec);
	if(wait_res>=WAIT_OBJECT_0 && wait_res<WAIT_OBJECT_0+MAXIMUM_WAIT_OBJECTS)
		return wait_res - WAIT_OBJECT_0;
	else
...
inline
int wait4event(HANDLE _h1,HANDLE _h2,HANDLE _h3,HANDLE _h4,DWORD _millisec)
{
	HANDLE h[] = {_h1,_h2,_h3,_h4};
	DWORD wait_res = ::WaitForMultipleObjects(_countof(h),h,FALSE,_millisec);
	if(wait_res>=WAIT_OBJECT_0 && wait_res<WAIT_OBJECT_0+MAXIMUM_WAIT_OBJECTS)
		return wait_res - WAIT_OBJECT_0;
	else
...
inline
int wait4event(HANDLE _h1,HANDLE _h2,HANDLE _h3,HANDLE _h4,HANDLE _h5,DWORD _millisec)
{
	HANDLE h[] = {_h1,_h2,_h3,_h4,_h5};
	DWORD wait_res = ::WaitForMultipleObjects(_countof(h),h,FALSE,_millisec);
	if(wait_res>=WAIT_OBJECT_0 && wait_res<WAIT_OBJECT_0+MAXIMUM_WAIT_OBJECTS)
		return wait_res - WAIT_OBJECT_0;
	else
...
public:
	Shuffle(size_t _idx0,size_t _idx1)
		:base(0)
	{
		VERIFY_EXIT(_idx0<_idx1);
		base = _idx0;
		idxs.resize(_idx1 - _idx0);
		size_t i = 0,j=0;
		for(i=0,j=_idx0;i<idxs.size();i++,j++)
		{
			idxs[i] = j;
		}
	...
	size_t operator [] (size_t _idx)
	{
		VERIFY_EXIT1(_idx>=base && _idx-base<idxs.size(),base);
		return idxs[_idx-base];
	}
};
...
protected:
	std::vector<Item> iterators;

public:
	SortIterators(_Iterator _it,_Iterator _ite)
	{
		iterators.resize(std::distance(_it,_ite));
		size_t i = 0;
		for(i=0;i<iterators.size() && _it!=_ite;++_it,i++)
		{
			iterators[i].it = _it;
		}
	}

	void sort(bool _ascending = true)
	{
		if(_ascending)
			std::sort(iterators.begin(),iterators.end());
		else
			std::sort(iterators.begin(),iterators.end(),std::greater<Item>());
	}

	_Iterator operator [] (size_t _idx)
	{
		VERIFY_EXIT1(_idx<iterators.size(),_Iterator());
		return iterators[_idx].it;
	}
	...
	double sum() const 
	{
		...
		{
			double val = (double)*(iterators[i].it);
			if(val>0) sum += val;
		}
		return sum;
	}

	_Iterator get(double _val,bool _ascending = true) const 
	{
		double sum = 0;
		long i = 0;
		if(_ascending)
		{
			for(i=0;i<(long)iterators.size();i++)
			{
				double val = (double)*(iterators[i].it);
				if(val>0) sum += val;
				if(_val<=sum) return iterators[i].it;
			}
			return iterators.front().it;
		}
		else
		{
			for(i=(long)iterators.size()-1;i>=0;i--)
			{
				double val = (double)*(iterators[i].it);
				if(val>0) sum += val;
				if(_val<=sum) return iterators[i].it;
			}
			return iterators.back().it;
		}
	}
};
...
_It find_low_bound(_It _it,_It _ite,_Value _value)
{
	typename std::iterator_traits<_It>::difference_type i_low = 0;
	typename std::iterator_traits<_It>::difference_type i_upper  = std::distance(_it,_ite);
	for(;i_low!=i_upper;)
	{
		typename std::iterator_traits<_It>::difference_type i_middle = (i_low+i_upper)/2;
		if(i_middle==i_low) break;
		_It it_middle = _it;
		std::advance(it_middle,i_middle);
		if(*it_middle==_value)
		{
			i_low = i_middle;
			break;
		}
		else if(*it_middle<_value)
			i_low = i_middle;
		else 
			i_upper = i_middle;
	}
	_It res = _it;
	std::advance(res,i_low);
	return res;
}

#if defined(DETECT_DEAD_LOCKS)
#define SOURCE_INFO() ((LPCTSTR)Format(_T("%s:%d"),(LPCTSTR)_T(__FILE__),__LINE__))
#else
#define SOURCE_INFO() (NULL)
#endif

template<typename _Type>
struct ptrvector: public std::vector<_Type*>
{
	...
	{
		delete trace_free(*_it);
		return base::erase(_it);
	}
};
