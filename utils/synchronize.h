...
protected:
	...
	CCriticalSection_ m_critsec;
	std::map<CString,LPVOID> m_sources;
#endif
	...
	static LockInfo& get_instance()
	{
		static LockInfo _;
		return _;
	}
public:
	static void add(LPCTSTR _szInfo,LPVOID _obj);
	static void remove(LPCTSTR _szInfo);
};
...
struct CAutoLockT<CCriticalSection_>
{
	...
	*/
	CAutoLockT(CCriticalSection_& _critsec,LPCTSTR _szSourceInfo = NULL)
	{
		m_phCritSect = GetCriticalSectionStruct(_critsec);
		if(NOT_NULL(_szSourceInfo))
		{
			m_source_info = CString(_szSourceInfo);
			LockInfo::add(m_source_info,m_phCritSect);
		}
	...
	\param _critsec -- critical section object used for synchronization
	*/
	CAutoLockT(const CCriticalSection_& _critsec,LPCTSTR _szSourceInfo = NULL)
	{
		m_phCritSect = GetCriticalSectionStruct(_critsec);
		if(NOT_NULL(_szSourceInfo))
		{
	...
	*/
	CAutoLockT(CRITICAL_SECTION& hCritSect,LPCTSTR _szSourceInfo = NULL)
	{
	...
	\param hCritSect -- critical section object used for synchronization
	*/
	CAutoLockT(const CRITICAL_SECTION& hCritSect,LPCTSTR _szSourceInfo = NULL)
	{
	...
	*/
	~CAutoLockT()
	{
		...
		if(!m_source_info.IsEmpty())
			LockInfo::remove(m_source_info);
	}
protected:
	CRITICAL_SECTION* m_phCritSect;		///< critical section object pointer
	CString m_source_info;
};

typedef CAutoLockT<CCriticalSection_> CAutoLock;			///< typedef of historical name

inline
void LockInfo::add(LPCTSTR _szInfo,LPVOID _obj)
{
#if defined(DETECT_DEAD_LOCKS)
	CAutoLock __al(get_instance().m_critsec,NULL);
	CString info(_szInfo);
	if(get_instance().m_sources.find(info)==get_instance().m_sources.end())
		get_instance().m_sources.insert(std::pair<CString,LPVOID>(info,_obj));
#endif
}

inline
void LockInfo::remove(LPCTSTR _szInfo)
{
#if defined(DETECT_DEAD_LOCKS)
	CAutoLock __al(get_instance().m_critsec,NULL);
	CString info(_szInfo);
	std::map<CString,LPVOID>::iterator it = get_instance().m_sources.find(info);
	if(it!=get_instance().m_sources.end())
		get_instance().m_sources.erase(it);
#endif
}
