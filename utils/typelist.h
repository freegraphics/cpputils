...
struct TypeListSize
{
	...
	enum {get = TypeListSize<Tail>::get + 1};
};

template<>
struct TypeListSize<NullType>
{
	enum {get = 0};
};

template<typename _TL,size_t _pos>
struct TypeAt
{
	typedef typename TypeAt<typename _TL::Tail,_pos-1>::res res;
};

template<typename _TL>
struct TypeAt<_TL,0>
{
	typedef typename _TL::Item res;
};
...
struct NewClass
{
	...
	static _RetType* get(size_t _idx,_RetType* _arg)
	{
		if(_idx==0) return new Item();
		else return NewClass<Tail>::get(_idx-1,_arg);
	}
};

template<>
struct NewClass<NullType>
{
	template<typename _RetType>
	static _RetType* get(size_t _idx,_RetType*) 
	{
		return NULL;
	}
};
