...
struct CUnitTestsOverStdOut : public IUnitTest
{
	...
	*/
		...
		)
	{
		...
		std::tcerr << _szFile << _T("(") << _nLine << _T("):") 
			<< _szDescription 
			<< std::endl
	...
	*/
	void result()
	{
		std::tcerr << _T("All tests count :") << m_AllTestsCount << std::endl
		...
		if(m_FailedTestsCount==0)
			std::tcerr << _T("All tests was successfully passed") << std::endl;
		else
			std::tcerr << _T("There was error while tests passing") << std::endl;
