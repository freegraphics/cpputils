	...
	,public ISerializeVisitBase
{
	...
	*/
	...
	\return \ref INode pointer of the root item for that archive
	*/
...
template<typename _StructType,typename _KeyType,typename _ValueType>
struct MapFieldType : public IMetaClass<_StructType>
{
	typedef std::map<_KeyType,_ValueType> Container;
	typedef TYPENAME Container::value_type value_type;
	typedef TYPENAME Container::_Pairib _Pairib;

	MapFieldType(
		const CString_& _nameid
		,Container _StructType::* _memberptr
		,const IMetaClass<_KeyType>& _metaitem_key
		,const IMetaClass<_ValueType>& _metaitem_value
		)
		:m_nameid(_nameid)
		,m_memberptr(_memberptr)
		,m_metaitem_key(_metaitem_key)
		,m_metaitem_value(_metaitem_value)
	{
	}
	...
	virtual void save(INode& _node,const _StructType& _pD) const
	{
		CPtrShared<INodeList> pnodelist = _node.get_childlist(m_nameid);
		VERIFY_EXIT(NOT_NULL(pnodelist));
		const Container& cont = _pD.*m_memberptr;
		long i = 0, cnt = (long)cont.size();
		pnodelist->alloc(cnt);
		...
		for(i=0;it!=ite;++it,i++)
		{
			CPtrShared<INode> pnode = (*pnodelist)[i];
			VERIFY_DO(NOT_NULL(pnode),continue);
			const TYPENAME std::map<_KeyType,_ValueType>::value_type& val = *it;
			CPtrShared<INode> pnode_sub = pnode->get_child(first_name());
			if(!pnode_sub) pnode_sub = pnode->add_child(first_name());
			if(pnode_sub) m_metaitem_key.save(*pnode_sub,val.first);
			pnode_sub = pnode->get_child(second_name());
			if(!pnode_sub) pnode_sub = pnode->add_child(second_name());
			if(pnode_sub) m_metaitem_value.save(*pnode_sub,val.second);
		}
	}
	...
	virtual void load(const INode& _node,_StructType& _pD) const
	{
		CPtrShared<INodeList> pnodelist = _node.get_childlist(m_nameid);
		VERIFY_EXIT(NOT_NULL(pnodelist));
		Container& cont = _pD.*m_memberptr;
		long i = 0, cnt = pnodelist->size();
		cont.clear();
		for(i=0;i<cnt;i++)
		{
			CPtrShared<INode> pnode = (*pnodelist)[i];
			VERIFY_DO(NOT_NULL(pnode),continue);
			CPtrShared<INode> pnode_sub = pnode->get_child(first_name());
			_KeyType key = _KeyType();
			if(pnode_sub) m_metaitem_key.load(*pnode_sub,key);
			pnode_sub = pnode->get_child(second_name());
			_ValueType value = _ValueType();
			if(pnode_sub) m_metaitem_value.load(*pnode_sub,value);
			cont.insert(value_type(key,value));
		}
	}

protected:
	static LPCTSTR first_name() {return _T("key");}
	static LPCTSTR second_name() {return _T("value");}

protected:
	CString m_nameid;
	Container _StructType::* m_memberptr;
	const IMetaClass<_KeyType>& m_metaitem_key;
	const IMetaClass<_ValueType>& m_metaitem_value;
};//template<> struct MapFieldType
...
#define DECLARE_FACTORY_CLASS_TRAITS(_ClassType)	\
	template<>	\
struct FactoryClassTraits<_ClassType>	\
{	\
	static LPCTSTR get_name()	\
{	\
	return _T(#_ClassType);	\
}	\
};
...
namespace Private
{
	...
	template<typename _Interface,typename _TypeList>
	struct ProcessFactoryTypeList
	{
		typedef TYPENAME _TypeList::Item CurrentItem;
		typedef TYPENAME _TypeList::Tail Tail;
		typedef _Interface* InterfaceItem;

		static void load(const INode& _node,InterfaceItem& _rule)
		{
			CPtrShared<INodeList> pnodelist = _node.get_childlist(CurrentItem::struct_name());
			if(pnodelist->size()>0)
			{
				CPtrShared<INode> pnode = (*pnodelist)[0];
				if(pnode)
				{
					_rule = trace_alloc(new CurrentItem());
					get_structlayout<CurrentItem>().load(*pnode,*static_cast<CurrentItem*>(_rule));
				}
				return;
			}
			else
				ProcessFactoryTypeList<_Interface,Tail>::load(_node,_rule);
		}

		static void save(INode& _node,const InterfaceItem& _rule)
		{
			if(IS_NULL(_rule)) return;
			if(_rule->get_struct_name()==CurrentItem::struct_name())
			{
				CPtrShared<INode> pnode = _node.get_child(CurrentItem::struct_name());
				if(!pnode) pnode = _node.add_child(CurrentItem::struct_name());
				if(pnode)
					get_structlayout<CurrentItem>().save(*pnode,*static_cast<const CurrentItem*>(_rule));
				return;
			}
			else
				ProcessFactoryTypeList<_Interface,Tail>::save(_node,_rule);
		}
	};

	template<typename _Interface>
	struct ProcessFactoryTypeList<_Interface,NullType>
	{
		typedef _Interface* InterfaceItem;
		static void load(const INode& _node,InterfaceItem& _rule)
		{
...
template<typename _Interface>
class FactoryLayout : public IMetaClass<_Interface*>
{
protected:
	typedef _Interface* Item;

	template<typename _TypeList>
	void load_by_typelist(const INode& _node,Item& _rule,_TypeList) const 
	{
		Private::ProcessFactoryTypeList<_Interface,_TypeList>::load(_node,_rule);
	}

	template<typename _TypeList>
	void save_by_typelist(INode& _node,const Item& _rule,_TypeList) const 
	{
		Private::ProcessFactoryTypeList<_Interface,_TypeList>::save(_node,_rule);
	}

public:

//	virtual void save(INode& _node,const Item& _item) const
//	{
//	}
//
//	virtual void load(const INode& _node,Item& _item) const
//	{
//	}
};
...
public:
	...
	*/
			...
			)
	{
		...
		template<typename _ItemType>
		void add_ptrvector(const FieldName<_StructType>& _nameid
			,ptrvector<_ItemType> _StructType::* _memberptr
			,const FactoryLayout<_ItemType>& _metaclass
			)
		{
			CString_ sName;
			_nameid.get_name(reinterpret_cast<void* _StructType::*>(_memberptr),sName);
			push_back(
				trace_alloc(
						(
							new ContainerFieldType<_StructType,ptrvector<_ItemType> >
							(
								sName
								,_memberptr
								,_metaclass
	...
	*/
	...
	template<typename _KeyItem,typename _ValueItem>
		void add_map(const FieldName<_StructType>& _nameid
			,std::map<_KeyItem,_ValueItem> _StructType::* _memberptr
			,const IMetaClass<_KeyItem>& _metaclass_key
			,const IMetaClass<_ValueItem>& _metaclass_value
			)
	{
		CString_ sName;
		_nameid.get_name(reinterpret_cast<void* _StructType::*>(_memberptr),sName);
		push_back(
				trace_alloc(
					(
					new MapFieldType<_StructType, _KeyItem, _ValueItem>
						(
						sName
						,_memberptr
						,_metaclass_key
		...
		void add_map(const FieldName<_StructType>& _nameid
			,std::map<_KeyItem,_ValueItem*> _StructType::* _memberptr
			,const IMetaClass<_KeyItem>& _metaclass_key
			,const FactoryLayout<_ValueItem>& _metaclass_value
			)
		{
			CString_ sName;
			_nameid.get_name(reinterpret_cast<void* _StructType::*>(_memberptr),sName);
		...
		void add_map(const FieldName<_StructType>& _nameid
			,std::map<_KeyItem*,_ValueItem> _StructType::* _memberptr
			,const FactoryLayout<_KeyItem>& _metaclass_key
			,const IMetaClass<_ValueItem>& _metaclass_value
			)
		{
			CString_ sName;
...
template<typename _Interface>
struct DefaultFactoryLayoutTraits
{
	typedef DefaultLayoutInnerRealize DefaultFactoryLayoutRealize;
};
...
#define DECLARE_OUTER_DEFAULT_FACTORY_LAYOUT(_Struct,_DefaultFactoryLayoutClass)	\
template<>	\
struct DefaultFactoryLayoutTraits<_Struct>	\
{	\
	typedef _DefaultFactoryLayoutClass DefaultFactoryLayoutRealize;	\
};
...
namespace Private
{
	...
	template<typename _DefaultFactoryLayout>
	struct get_defaultfactorylayout
	{
		template<typename _Struct>
		const FactoryLayout<_Struct>& operator() (_Struct*)
		{
			return singleton<_DefaultFactoryLayout>::get();
		}
	};

	template<>
	struct get_defaultfactorylayout<DefaultLayoutInnerRealize>
	{
		template<typename _Struct>
		const FactoryLayout<_Struct>& operator() (_Struct*)
		{
			typedef typename _Struct::DefaultFactoryLayout DefaultFactoryLayout;
			static const DefaultFactoryLayout _;
			return _;
		}
	};
};//namespace Private
...
const IMetaClass<_Struct>& get_structlayout()
{
	...
	return Private::get_defaultlayout<DefaultLayoutRealize>()((_Struct*)NULL);
}

template<typename _Struct>
const FactoryLayout<_Struct>& get_factorylayout()
{
	typedef	typename DefaultFactoryLayoutTraits<_Struct>::DefaultFactoryLayoutRealize DefaultFactoryLayoutRealize;
	return Private::get_defaultfactorylayout<DefaultFactoryLayoutRealize>()((_Struct*)NULL);
}
...
#define GETSTRUCTNAME()	\
public:	\
virtual LPCTSTR get_struct_name() const\
{	\
	return struct_name();	\
}

#define DECLARE_GETSTRUCTNAME()	\
	public:	\
	virtual LPCTSTR get_struct_name() const = 0;
